...
function renderTrending(){const grid=document.querySelector('#trending-grid');grid.replaceChildren();data.shows.slice(0,12).forEach(show=>{const link=document.createElement('a');link.className='show';link.href=`https://www.themoviedb.org/tv/${show.id}`;link.target='_blank';link.rel='noopener';const image=document.createElement('img');image.src=show.poster;image.alt='';image.loading='lazy';const heading=document.createElement('h3');heading.textContent=show.title;const rank=document.createElement('p');rank.textContent=`Trending · ${show.rank}`;link.append(image,heading,rank);grid.append(link)});const updated=new Date(data.updatedAt);document.querySelector('#freshness').textContent=`Updated ${updated.toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'})} · TMDB`}
try{
