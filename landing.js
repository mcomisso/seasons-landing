...
const copy=[
  ['Your streaming universe.<br><span>One centre.</span>','Seasons tells you exactly when to subscribe and cancel, so you only pay for what you love.'],
  ['Everything revolves<br><span>around you.</span>','Your shows and streaming services come together in one clear view.'],
  ['Less searching.<br><span>More looking forward.</span>','See what you want to watch, then plan when each subscription is worth keeping.']
];
const progress=()=>clamp(scrollY/Math.max(1,journey.offsetHeight-innerHeight));
const clamp=value=>Math.max(0,Math.min(1,value));
function render(){pending=false;const value=reduced?1:progress(),next=value<.31?0:value<.76?1:2;if(next!==phase){phase=next;title.innerHTML=copy[phase][0];description.textContent=copy[phase][1]}scene?.update(value)}
...
function renderTrending(){const grid=document.querySelector('#trending-grid');grid.replaceChildren();data.shows.slice(0,12).forEach(show=>{const card=document.createElement('article');card.className='show';const image=document.createElement('img');image.src=show.poster;image.alt='';image.loading='lazy';const heading=document.createElement('h3');heading.textContent=show.title;card.append(image,heading);grid.append(card)});const updated=new Date(data.updatedAt);document.querySelector('#freshness').textContent=`Updated ${updated.toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'})} · TMDB`}
try{
